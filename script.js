	...
	(function($) {
		...
		function html($this, time, opts) {
			...
			if(opts.labelsFormat === true && typeof window['regexpMatchFormat_' + $this.attr('id')] === 'undefined' && typeof window['regexpReplaceWith_' + $this.attr('id')] === 'undefined') {
				$this.addClass("labelformat");
				time = time.split(opts.timeSeparator);
				var labelTime = "<span class='timerDisplay label" + time.length + "'>";
				var labelarr = [];
				for(var j = 0; j < 6; j++) {
					if(format[j] === "!") labelarr.push(opts.labels[j]);
				}
				...
				time = labelTime += "</span>";
			} else if(opts.labelsFormat === false && typeof window['regexpMatchFormat_' + $this.attr('id')] !== 'undefined' && typeof window['regexpReplaceWith_' + $this.attr('id')] !== 'undefined') {
				var regexp = new RegExp(window['regexpMatchFormat_' + $this.attr('id')]);
				time = time.replace(regexp, window['regexpReplaceWith_' + $this.attr('id')]);
			}
			$this.html(time);
		}
	...
	$(window).load(function() {
		$("#loader").fadeOut(), $("#preloader").delay(0).fadeOut("slow"), $("body").delay(0).css({
			overflow: "visible"
		})
	});
	$(function() {
	...
	$("ul#related-summary li img").each(function() {
		$(this).attr("src", $(this).attr("src").replace('s72-c', 's600'))
	});
	$(".avatar-image-container img").each(function() {
		$(this).attr("src", $(this).attr("src").replace(/\/s[0-9]+(\-c)?\//, "/w70-h70-c/"))
	});
